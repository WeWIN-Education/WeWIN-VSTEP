...
export default async function ReviewPage() {
  ...
  return <div className="mx-auto w-full max-w-[820px] space-y-6"><PageHero eyebrow="LUYỆN TẬP TỔNG HỢP" title="Một phiên ôn trộn" description="Câu hỏi được chọn ngẫu nhiên từ các kho Listening và Reading đã xuất bản. Làm xong xem kết quả ngay, không lưu vào lịch sử." stats={[{ label: "Câu trong phiên", value: String(items.length) }, { label: "Kỹ năng", value: "2" }, { label: "Mục tiêu", value: "Ôn lại" }]} />{items.length ? <PracticePlayer type="MIXED" items={items} /> : <Card padding="lg"><p className="text-sm text-ink-muted">Chưa có câu hỏi Listening hoặc Reading đã xuất bản để tạo phiên ôn.</p></Card>}</div>;
}
