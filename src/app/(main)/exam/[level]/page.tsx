...
export default async function ExamHubPage({ params, searchParams }: { params: Promise<{ level: string }>; searchParams: Promise<{ catalog?: string | string[] }> }) {
  ...
  const [exams, stats] = await Promise.all([
    prisma.examPaper.findMany({
      where: { programme: "VSTEP", status: { in: statuses as ("READY" | "SAMPLE" | "PUBLISHED")[] } },
      orderBy: [{ sortOrder: "asc" }, { createdAt: "desc" }],
      select: { slug: true, title: true, subtitle: true, target: true, durationMin: true, status: true },
    }),
    authState.kind === "authenticated"
      ? prisma.examAttempt.count({ where: { userId: authState.user.id, catalog, examPaper: { programme: "VSTEP" }, status: "SUBMITTED" } })
  ...
  return (
    <div className="mx-auto max-w-[1180px]">
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <PageHero eyebrow={config.eyebrow} title="Luyện thi VSTEP" description="Mỗi đề gồm Full Test và bốn kho kỹ năng để bạn luyện riêng." />
        <Card padding="lg" className="flex flex-col justify-between">
          <div><p className="text-xs font-bold text-ink-muted">{activeCatalog.label}</p><p className="mt-2 text-2xl font-extrabold text-brand">{progress === null ? "Học thử" : `${progress}%`}</p><p className="mt-1 text-sm leading-relaxed text-ink-muted">{progress === null ? "Khách được làm tối đa 2 đề khác nhau trong mỗi kho, mỗi đề một lượt." : stats ? `${stats} lượt đã nộp trong kho này.` : activeCatalog.description}</p></div>
          {exams[0] ? <Link href={`/exam/vstep/${exams[0].slug}?catalog=${catalog}`} className="mt-4 flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius-btn)] bg-brand px-4 text-sm font-extrabold text-white transition hover:bg-brand-dark">{progress === null ? "Học thử ngay" : "Mở đề đầu tiên"}<ArrowRight className="size-4" /></Link> : <span className="mt-4 flex min-h-11 items-center justify-center rounded-[var(--radius-btn)] border border-border bg-surface px-4 text-sm font-bold text-ink-muted">Chưa có đề mở</span>}
        </Card>
      </div>
