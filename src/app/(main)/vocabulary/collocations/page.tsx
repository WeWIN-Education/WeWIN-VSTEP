...
export default async function VocabularyCollocationsPage({ searchParams }: { searchParams: Promise<{ q?: string | string[] }> }) {
  ...
  if (!user) return <div className="mx-auto w-full max-w-[1120px] space-y-6"><PageHero eyebrow="TỪ VỰNG · CỤM TỪ" title="Cụm từ & collocations" description="Kho cụm từ được quản trị viên nhập theo workbook. Đăng nhập để tìm kiếm, học flashcard và lưu tiến độ." /><LoginGate title="Đăng nhập để mở kho cụm từ" description="Tài khoản WEWIN mới mở nội dung học và trạng thái ôn tập." callbackUrl="/vocabulary/collocations">{null}</LoginGate></div>;
