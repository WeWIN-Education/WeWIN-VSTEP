...
type HistoryData = { xp: number; total: number; items: Array<{ slot: number; score: number; correct: number; reward: { xp: number } | null; match: { id: string; status: string; reason: string | null; winnerSlot: number | null; finishedAt: string } }> };
function useBattle(matchId?: string, enabled = true) {
  const [state, setState] = useState<State | null>(null);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
  const sending = useRef(false), heartbeatAt = useRef(0), offset = useRef(0);
  const queuedAction = useRef(false);
  const [now, setNow] = useState(0);
  const send = useCallback(async (command: BattleCommand) => {
    if (command.action === "state" && (sending.current || queuedAction.current)) return false;
    if (command.action !== "state") {
      if (queuedAction.current) return false;
      queuedAction.current = true;
      setBusy(true);
      while (sending.current) await new Promise(resolve => setTimeout(resolve, 25));
    }
    sending.current = true;
    if (command.action !== "state") setBusy(true);
    try {
      const heartbeat = Date.now() - heartbeatAt.current >= 5000;
      const response = await fetch("/api/battle", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...command, matchId, heartbeat }), cache: "no-store", signal: AbortSignal.timeout(12_000) });
      ...
      return true;
    } catch (e) { setError(e instanceof Error ? e.message : "Mất kết nối. Đang thử lại; thời gian trận vẫn tiếp tục."); return false; }
    finally { sending.current = false; if (command.action !== "state") { queuedAction.current = false; setBusy(false); } }
  }, [matchId]);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => { void send({ action: "state" }); }, 1000);
  ...
  useEffect(() => {
    const id = setInterval(() => setNow(Date.now() + offset.current), 150);
    return () => clearInterval(id);
  }, []);
  ...
  return <div className={styles.lobby}>
    <section className={styles.hero}>
      <div className={styles.heroCopy}><span className={styles.eyebrow}><Gamepad2 size={18} aria-hidden /> WEWIN ARENA · B1</span><h1>Học một chút.<br /><em>Đấu một trận!</em></h1><p>Quick Battle 1vs1. Thử phản xạ tiếng Anh qua từ vựng, ngữ pháp và cụm từ.</p>
        <div className={styles.tags}><span>30 lượt</span><span>Tối đa 15 giây / câu</span><span>Đấu nhanh 1vs1</span></div>
        {!authenticated ? <Link className={styles.primary} href="/login?callbackUrl=/game">Đăng nhập để chơi <Swords size={20} aria-hidden /></Link>
          ...
          : <button className={styles.primary} disabled={!enabled || busy || !state || state.kind === "match"} onClick={() => void send({ action: "join" })}>{busy ? "Đang tìm…" : enabled ? "Tìm trận ngay" : "Đang chuẩn bị mở đấu"}<Swords size={20} aria-hidden /></button>}
    ...
    <div className={styles.lobbyGrid}>
      ...
      <section className={styles.panel}><span className={styles.eyebrow}><Swords size={18} aria-hidden /> LUẬT ĐẤU</span><h2>Phản xạ nhanh. Hiểu thật chắc.</h2><ul className={styles.rules}>
        <li>Luân phiên 15 câu mỗi người: 5 từ vựng, 5 ngữ pháp, 5 cụm từ. Người đi trước ngẫu nhiên.</li>
        <li>Đúng: 100–150 điểm theo tốc độ; sai hoặc hết 15 giây: 0 điểm. Chọn đáp án là biết kết quả, sau 2 giây chuyển sang câu tiếp theo.</li>
    ...
    {authenticated && <section className={styles.panel}><h2>Lịch sử trận đấu</h2>{historyError && <p role="alert">{historyError}</p>}{!history && !historyError && <p>Đang tải lịch sử…</p>}{history?.items.length === 0 && <p>Trận đầu tiên đang chờ bạn.</p>}
      <div className={styles.history}>{history?.items.map(item => <Link key={item.match.id} href={`/battle/${item.match.id}`}><span>{new Date(item.match.finishedAt).toLocaleString("vi-VN")}</span><strong>{outcome(item.match.status, item.match.winnerSlot, item.slot)}</strong><span>{item.score} điểm · {item.correct}/15 đúng</span><b>+{item.reward?.xp ?? 0} XP</b></Link>)}</div>
...
export function BattleArena({ matchId }: { matchId: string }) {
  ...
  if (!match) return <main className={styles.arenaPage}><Link href="/game">← Về sảnh</Link><h1>Đang kết nối sân đấu…</h1>{error && <p role="alert">{error}</p>}</main>;
  ...
  return <main className={styles.arenaPage}>
    ...
    <div className={styles.fightLayout}>
      <section className={styles.stage} aria-label="Sân đấu">
        <div className={styles.scoreboard}>{[me, opponent].map(p => <div key={p.slot}><RankBadge rank={p.rank} size={64} /><span>{p.name}</span><small>{p.slot === match.mySlot ? "Bạn" : "Đối thủ"} · {RANKS[p.rank]?.name}</small><strong>{p.score.toLocaleString("vi-VN")}</strong></div>)}</div>
        ...
        <div className={styles.stageStatus} role="status">{finished ? outcome(match.status, match.winnerSlot, match.mySlot) : countdown ? `Sẵn sàng! ${countdown}` : turn?.answered ? correct ? "Chính xác! Một đòn tấn công đẹp." : "Chưa chính xác · Xem giải thích để ghi nhớ." : mine ? "Đến lượt bạn!" : `${opponent.name} đang trả lời…`}</div>
        <div className={styles.turns} aria-label={`Tiến độ: lượt ${(turn?.number ?? -1) + 1} trên 30`}>{Array.from({ length: 30 }, (_, i) => <span key={i} data-done={finished || i < (turn?.number ?? 0)} data-current={!finished && i === turn?.number}>{i + 1}</span>)}</div>
      </section>
      <section className={styles.questionPanel}>
        ...
        {finished ? <><span className={styles.eyebrow}><Trophy size={20} aria-hidden /> KẾT QUẢ</span><h1>{outcome(match.status, match.winnerSlot, match.mySlot)}</h1><div className={styles.resultScore}>{me.score}<small>điểm</small></div><p>{me.correct}/15 câu đúng · <strong>+{me.xp} XP thực nhận</strong></p>{match.reason !== "COMPLETE" && <p>{match.reason === "BOTH_OFFLINE" ? "Cả hai mất kết nối quá 30 giây. Trận không có thưởng." : "Trận kết thúc vì một người bỏ cuộc hoặc mất kết nối."}</p>}{me.xp === 0 && <p>Không có XP: trận không đủ điều kiện thưởng hoặc đã đạt 5 trận có thưởng trong ngày.</p>}<Link href="/game" className={styles.primary}>Đấu ván nữa <Swords size={20} aria-hidden /></Link><Link href="/game" className={styles.backLink}>Về sảnh và xem rank mới</Link></>
          : !turn || countdown ? <div className={styles.countdown}><Shield size={40} aria-hidden /><h1>Sẵn sàng giao đấu</h1><strong>{countdown || 3}</strong><p>30 lượt · 15 câu mỗi người</p></div>
          : <><div className={styles.questionTop}><span>{TYPE_LABELS[turn.type as BattleType]} · {turn.number + 1}/30</span><span className={styles.timer} data-urgent={!turn.answered && seconds <= 5}><Clock3 size={20} aria-hidden />{turn.answered ? `Chuyển ${Math.max(0, Math.ceil((turn.nextAt - now) / 1000))}s` : `${seconds}s`}</span></div><h1>{turn.prompt}</h1><p className={styles.turnLabel}>{turn.answered ? "Đã ghi nhận đáp án" : mine ? "Chọn một đáp án" : "Lượt của đối thủ · Bạn đang theo dõi"}</p>
            <div className={styles.options}>{turn.options.map((option, i) => <button key={i} data-result={turn.correct === i ? "correct" : turn.choice === i ? "wrong" : undefined} data-selected={selected === i} disabled={!mine || busy || turn.answered || seconds === 0} onClick={async () => { setSelected(i); if (!await send({ action: "answer", turn: turn.number, choice: i })) setSelected(null); }}><span>{String.fromCharCode(65 + i)}</span>{option}{turn.correct === i ? <Check aria-label="Đáp án đúng" size={20} /> : turn.choice === i ? <X aria-label="Đáp án sai" size={20} /> : null}</button>)}</div>
            {turn.explanation && <div className={styles.explanation}><strong>{correct ? "Đúng" : "Sai hoặc hết giờ"}</strong><p>{turn.explanation}</p></div>}
            {!turn.answered && selected !== null && <p role="status">Đang gửi đáp án…</p>}
          </>}
      </section>
    </div>
    {finished && <section className={`${styles.panel} ${styles.review}`}><h2>Ôn lại trận đấu</h2><p>Bộ câu hỏi WEWIN tự biên soạn theo định hướng VSTEP B1.</p>{match.review.map(q => <details key={q.number}><summary><span>{q.number + 1}. {q.prompt}</span><b>{q.slot === match.mySlot ? "Bạn" : "Đối thủ"} · {q.points! > 0 ? "Đúng" : "Sai / bỏ trống"}</b></summary><p>Đã chọn: {q.choice === null || q.choice === undefined ? "Không trả lời" : q.options[q.choice]}</p><p><strong>Đáp án: {q.options[q.correct!]}</strong></p><p>{q.explanation}</p></details>)}</section>}
