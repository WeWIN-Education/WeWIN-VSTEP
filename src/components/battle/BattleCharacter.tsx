...
export function BattleCharacter({ variant = "hero", state = "idle", facing = "right", paused = false, replay = 0, rank = 5 }: {
  ...
  useEffect(() => {
    ...
    let visible = true, disposed = false, loaded = false;
    ...
    const update = () => {
      ...
      renderer?.setRunning(!inactive && loaded);
    };
    ...
    image.onload = () => {
      if (disposed) return;
      if (renderer) {
        renderer.load(image, CHARACTER_STATES.findIndex((item) => item.state === state), characterHands(variant, state));
  ...
  return (
    <div ref={root} className={styles.character} data-state={state} data-variant={variant} data-paused={paused} data-rank={rank} role="img" aria-label={`${variant === "hero" ? "Anh hùng" : "Đối thủ"}: ${label}`} style={{ "--direction": facing === "left" ? -1 : 1 } as CSSProperties}>
      ...
      <div key={`${state}-${replay}`} className={styles.action} aria-hidden="true">
