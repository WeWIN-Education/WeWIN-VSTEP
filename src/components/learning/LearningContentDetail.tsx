...
import { parseLearningExercise } from "@/lib/learning-exercise";
...
export async function LearningContentDetail({ kind, id }: { kind: ContentKind; id: string }) {
  ...
  if (!item) notFound();
  const exercise = kind === "EXERCISE" ? parseLearningExercise(item.body, item.skill) : null;
  ...
  return <div className="mx-auto max-w-[900px] space-y-5">
    ...
    {exercise ? <LearningExercisePlayer exercise={exercise} audioSrc={item.audioKey ? `/api/learning-content/${item.id}/audio` : undefined} /> : <>
