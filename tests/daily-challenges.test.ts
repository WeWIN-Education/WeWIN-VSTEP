...
const mocks = vi.hoisted(() => ({ count: vi.fn(), aggregate: vi.fn(), find: vi.fn(), create: vi.fn(), update: vi.fn(), transaction: vi.fn(), actor: vi.fn() }));
vi.mock("server-only", () => ({}));
vi.mock("@/lib/access", () => ({ getCurrentUser: mocks.actor }));
vi.mock("@/lib/prisma", () => ({ prisma: {
  vocabularyProgress: { count: mocks.count }, xpAward: { aggregate: mocks.aggregate },
...
beforeEach(() => {
  ...
  mocks.aggregate.mockResolvedValue({ _sum: { amount: 150 } }); mocks.find.mockResolvedValue(null);
  mocks.transaction.mockImplementation(async fn => fn({ vocabularyProgress: { count: mocks.count }, xpAward: { aggregate: mocks.aggregate }, dailyChallengeReward: { findUnique: mocks.find, create: mocks.create }, user: { update: mocks.update } }));
});
...
it("bounds progress and excludes reward XP by reading only exam awards", async () => {
  expect(dailyChallengeTasks(99, 200).map(task => task.value)).toEqual([30, 50, 150]);
  expect(dailyChallengeTasks(-1, 0).map(task => task.value)).toEqual([0, 0, 0]);
  ...
  expect(mocks.aggregate.mock.calls[0][0].where.userId).toBe("user-a");
});
